...
TEST_CASE("Check refinement/unrefinement polyhedral AMR/LB functionality for "
          "adaptiveFvMesh",
  ...
  IStringStream is("dynamicFvMesh   adaptiveFvMesh;"
  ...
  // Create mesh object
  IOdictionary dynamicMeshDict(IOobject("dynamicMeshDict", runTime.constant(),
                                        runTime, IOobject::NO_READ,
                                        IOobject::NO_WRITE),
                               is);
  dynamicMeshDict.regIOobject::write();
  adaptiveFvMesh mesh(IOobject("", runTime.constant(), runTime,
                               IOobject::MUST_READ, IOobject::NO_WRITE));

  // Capture test variables for sane reports
  CAPTURE(Pstream::myProcNo(), Pstream::nProcs(), runTime.caseName(),
          mesh.nCells(), balance, refiner, maxRefL, nBufferLayers);

  // Create AMR criterion field
  volScalarField test(IOobject("test", runTime.timeName(), mesh,
                               IOobject::NO_READ, IOobject::NO_WRITE, true),
                      mesh, dimensionedScalar("zero", dimless, 0.0));

  // Set test to 1 inside the refined box
  forAll(mesh.C(), ci) {
    const auto &center = mesh.C()[ci];
    if (center.x() >= boxBounds[0] && center.x() <= boxBounds[1] &&
  ...
  volVectorField U(IOobject("U", runTime.timeName(), mesh, IOobject::NO_READ,
                            IOobject::NO_WRITE, true),
                   mesh, dimensionedVector("U", dimVelocity, vector(1, 2, 3)));
  ...
  // run a refinement cycle, minor unrefinement should happen here, but this not
  // checked.
  for (label i = 0; i < maxRefL; i++) {
    runTime++;
    tryAndCatchAbortingCode([&]() { mesh.update(); });
  }
  ...
  // Require that the new box gets refined to at least 4xmaxRefLx the original
  // cell count This is an inaccurate check; but there is nothing we can do.
  ...
  // re-evaluates it. Faces reversed by refinement need their flux negated,
  // which is what the flipFaceFlux flag in meshTools::modifyFace is for.
  {
    const surfaceScalarField phiU(fvc::interpolate(U) & mesh.Sf());
    scalar maxErr = 0;
    for (label facei = 0; facei < mesh.nInternalFaces(); facei++) {
      maxErr = max(maxErr,
                   mag(phi[facei] - phiU[facei]) / (mag(phiU[facei]) + SMALL));
    }
...
TEST_CASE("Check boundary protection for adaptiveFvMesh",
  ...
  word patchName = caseName.contains("poly2D") ? "left" : "fixedWalls";
